<?php $this->session->set_userdata('recipe_counter',1);?>
<script>
...
function validate()
{
	var error=0;
			...
			document.getElementById('first1').style.borderColor='#7F9DB9';
				document.getElementById('email1').style.borderColor='#7F9DB9';
	for(i=1;i<2;i++)
	{
		if(document.getElementById('first'+i).value=='')
		{
			error=1;
			document.getElementById('first'+i).style.borderColor='#FF0000';
			document.getElementById('err1'+i).style.display='block';
...
<form name="request" id="request" action="<?php echo base_url();?>index.php/email/shareFriend" method="post" class="standardForm" onsubmit="return validate();">
  ...
  <tr>
    <td width="90">Friend Name </td>
    <td><input type="text" name="first1" id="first1" value="" /><div id="err11" style="color:#F00; display:none; text-align:center">Please Enter Friend's Name</div></td>
    <td>Email</td>
 ...
 <!-- <tr>
  ...
  <tr>
    ...
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td><input type="submit" name="requestbtn" id="requestbtn" value="Send" class="calcipeButton" /></td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
  </tr>

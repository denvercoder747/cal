...
function lookup_multi(inputString,id,srch) {
	var link = "/index.php/gredient";
...
<div class="h1-edit"><span style="color:#CAD2B7">Edit Recipe</span></div> 
  	  <div id="add_recipe_left_part">
        	<div id="recipe_green_box">
        	  <div class="h2"><input id="recipe_name" name="recipe_name" type="text" value="<?php echo $row->name;?>" class="h2" style="width:535px" /><div id="ProcessBar" style="display:none">
            ...
            <div class="main-text">
            Photo of Recipe : <input type="file" name="userfile" id="userfile" /><input type="hidden" id="T3" name="T3" size="20" value="<?php echo $row->image;?>" />
            </div>
...
el.setAttribute("onkeyup","<?php if($row->multi_recipe=='True'){?>lookup_multi(this.value,"+iteration+",'searching_multi');<?php }else{?> lookup_multi(this.value,"+iteration+",'searching3');<?php }?>");
...
//    myNewField = document.createElement('<input name="tags"/>');
    ...
    e22 = document.createElement('<input type="checkbox" name="chk" style="display:none" bgcolor="#FFFFFF" id="chk'+iteration+'"/>');
		cellLeft.appendChild(e22);
    e1 = document.createElement('<input type="text" name="ingRow[]" class="calcipeMediumBox" onkeyup="<?php if($row->multi_recipe=='True'){?>lookup_multi(this.value,'+iteration+','+"'"+"searching_multi"+"'"+');<?php }else{?> lookup_multi(this.value,'+iteration+','+"'"+"searching3"+"'"+');<?php }?>" onfocus="hideMsg('+iteration+');" autocomplete="OFF" id="ingRow'+iteration+'"/>');
...
<input name="tot_row1" type="hidden" id="tot_row1" value="<?php echo $this->membership_model->num_gradients($row->recipe_id); ?>" />
      ...
      <tr>
        ...
        <input class="calcipeMediumBox" autocomplete="OFF" type="text" name="ingRow[]" id="ingRow<?php echo $incr;?>" size="15" value="<?php echo $grad->name; ?>"  onkeyup="<?php if($row->multi_recipe=='True'){?>lookup_multi(this.value,<?php echo $incr;?>,'searching_multi');<?php }else{?> lookup_multi(this.value,<?php echo $incr;?>,'searching3');<?php }?>" />
...
<td height="25" bgcolor="#69b1c1"><table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
    ...
    <td><input class="calcipeCal" name="infrastructure_cost" type="text" id="infrastructure_cost" value="<?php echo $row->infrastructure_cost; ?>" size="10" onblur="calculate();" style="text-align:right; color:#000; font-weight:bold;"  /></td>
  </tr>
</table></td>

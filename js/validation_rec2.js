...
//	alert('2');
		...
		if(ele.val().length < 3) {
			jVal.errors = true;
				nameInfo.removeClass('correct').addClass('error').html('&larr; at least 3 characters').show();
...
//				gredientInfo.removeClass('error').addClass('correct').html('&radic;').show();
			...
			for(j=1;j<=totrow;j++)
			{
				if($('#ingRow' + j) && $('#ingRow' + j).offset())
				{
				
				$('body').append('<div id="gredientInfo'+j+'" class="info"></div>');
				
				var ingredientInfo = $('#gredientInfo'+j);
				var ele = $('#ingRow' + j);
				var pos = ele.offset();
...
//				ingredientInfo.removeClass('error').addClass('correct').html('&radic;').show();
				...
				$('#ingRow' + j).removeClass("error");
						...
						$('body').append('<div id="mesInfo"'+ j +'class="info"></div>');
...
function hideMsg(id)
{
				var ingredientInfo = $('#gredientInfo'+id);
				var ele = $('#ingRow' + id);
//				ingredientInfo.removeClass('error').addClass('correct').html('&radic;').show();
//				ele.removeClass('wrong').addClass('normal');
//				$('#ingRow' + j).removeClass("error");

...
//cellRight.innerHTML="<img src='/images/minus_image.png' width='14' height='14' class='delete' onClick=\"deleteRow('tblSample');calculate();\" />";
//cellRight.innerHTML="<img src='/images/minus_image.png' width='14' height='14' class='delete' onClick=\"if (confirm('Are you sure you want to Remove'))removeRowFromTable("+iteration+");hideMsg2();calculate();\" />";
...
//		total_amt=total_amt+(parseFloat(document.getElementById('qtyRow'+i).value)*parseFloat(document.getElementById('rateRow'+i).value));
		...
		amtRow[i].value=parseFloat(amtrw.toFixed(4));
		measureRow[i].value=unitRow[i].options[unitRow[i].selectedIndex].text;
	}
	if(total<=0)return false;
	document.getElementById('tot_qty').value=total.toFixed(4);
	document.getElementById('tot_amt').value=total_amt.toFixed(4);
